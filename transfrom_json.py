...
def run():
    ...
    p.removeBody(object)
    flags=1
    json_file=initialization_json()
    #依次读取json文件内容进行仿真
    for i in range(100):
        #读取json文件，获得从夹爪坐标系到物体坐标系的变换矩阵，4x4,包括旋转和平移,flag_1代表着文件内是否还有有效数据
        grasp_center,grasp_endpoints,gravity,flag_1,obj_id,scene_id,cam_id=read_json(i)
        ...
        if flag_1==True:
            for j in range(num_points):
                ...
                flags,panda,object,log,object_pos_1,matrix_1=step(panda,object_T_global,grasp_center,grasp_endpoints,log,i,center_of_mass,obj_id,scene_id,cam_id,gravity)
                #逐渐减小机械臂的力，记录最小的夹取力
                # min_froce=decrease_froce(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,i,center_of_mass)
                # print(min_froce)
                #逐渐增加物体质量，返回最后掉落时的质量
...
#将世界坐标系中的点转换到物体坐标系中。
def globalpoint_T_object(global_point, object_T_global):
    # 将原坐标后面加一个1, 以便于计算齐次坐标
    global_point_new = np.hstack((global_point, 1))
    # 计算变换矩阵的逆矩阵 (从世界坐标系到物体坐标系)
    global_T_object = np.linalg.inv(object_T_global)
    # 将世界坐标系的点映射到物体坐标系
    object_point_new = np.dot(global_point_new, global_T_object.T)
    # 只取前三个坐标
    object_point = object_point_new[:3]
    return object_point
...
def step(panda,object_T_global,grasp_center,grasp_endpoints,log,opnum,center_of_mass,obj_id,scene_id,cam_id,gravity):
    ...
    p.setJointMotorControl2(panda, right_finger_id, p.POSITION_CONTROL, targetPosition=finger_max_position,
                            force=10, maxVelocity=target_velocity)
    ...
    object_pos_1,object_ori_1=p.getBasePositionAndOrientation(object)
    ...
    if len(contact_point_1)>0 and len(contact_point_2)>0:
            flags=True
            #计算此时接触点在物体坐标系中的位置，按照预定的格式输出为json
            output_json(panda,object,obj_id,scene_id,cam_id,gravity,center_of_mass)
            if flags==True:
                log.iloc[-1, 2] = angle
...
#计算此时接触点在物体坐标系中的位置，按照预定的格式输出为json
def output_json(panda,object,obj_id,scene_id,cam_id,gravity,center_of_mass):
    endpoints=[]
    vector=[]
    object_T_global=get_object_matrix(object,center_of_mass)
    # 处理 gravity 和 center_of_mass 的序列化
    gravity = tensor_to_serializable(gravity)
    end_effector_pos = p.getLinkState(panda, end_effector_id)[0]
    end_effector_pos = globalpoint_T_object(end_effector_pos,object_T_global)
    end_effector_pos = [tensor_to_serializable(c) for c in end_effector_pos]

    left_finger_pos = p.getLinkState(panda, left_finger_id)[0]
    right_finger_pos = p.getLinkState(panda, right_finger_id)[0]
    center=(np.array(left_finger_pos)+np.array(right_finger_pos))/2
    center=globalpoint_T_object(center,object_T_global)
    end_finger_pos = p.getLinkState(panda, end_effector_id)[0]
    finger_ori=np.array(left_finger_pos)-np.array(right_finger_pos)
    finger_ori=normalized(finger_ori)
    finger_1=end_finger_pos+0.2*finger_ori
    finger_2=end_finger_pos-0.2*finger_ori
    finger_1=globalpoint_T_object(finger_1,object_T_global)
    finger_2=globalpoint_T_object(finger_2,object_T_global)
    finger_1 = [tensor_to_serializable(c) for c in finger_1]
    finger_2 = [tensor_to_serializable(c) for c in finger_2]
    endpoints.append(finger_1)
    endpoints.append(finger_2)
    finger_ori_1=np.array(finger_2)-np.array(finger_1)
    finger_ori_2=np.array(finger_1)-np.array(finger_2)
    finger_ori_1=normalized(finger_ori_1)
    finger_ori_2=normalized(finger_ori_2)
    finger_ori_1 = [tensor_to_serializable(c) for c in finger_ori_1]
    finger_ori_2 = [tensor_to_serializable(c) for c in finger_ori_2]

    # finger_ori=globalpoint_T_object(finger_ori,object_T_global)
    # finger_ori_0=globalpoint_T_object([0,0,0],object_T_global)
    # finger_ori=finger_ori-finger_ori_0
    # finger_ori_1 = [tensor_to_serializable(-c) for c in finger_ori]
    # finger_ori_2 = [tensor_to_serializable(c) for c in finger_ori]
    vector.append(finger_ori_1)
    vector.append(finger_ori_2)
    data = {
        "scene_id": scene_id,
        "cam_id": cam_id,
        "obj_id": obj_id,
    ...
    # 将新数据写入json文件
    with open(f"{trans_json_path}.json", "r") as json_file:
        json_data = json.load(json_file)
    json_data.append(data) 
    with open(f"{trans_json_path}.json", "w") as json_file:
        json.dump(json_data, json_file, indent=4)

# 用于将 Tensor 或 ndarray 转换为可序列化的格式
def tensor_to_serializable(tensor):
    if isinstance(tensor, np.ndarray):
        return tensor.tolist()  # 将 numpy ndarray 转换为 Python 列表
    elif isinstance(tensor, (list)):
        return tensor  # 如果已经是 list，直接返回
    elif hasattr(tensor, 'item'):
        return tensor.item()  # 将 Tensor 转为标量值（float）
    return tensor  # 其他类型直接返回
...
#在稳定后逐渐减小机械臂的力
def decrease_froce(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,pose_num,center_of_mass):
    if flags==True:
        new_force=target_force
        for i in range(10000):
        # 每1秒减少10N的力
            new_force -= 5
            p.setJointMotorControl2(panda, left_finger_id, p.POSITION_CONTROL, targetPosition=0,
                            force=new_force, maxVelocity=target_velocity)
            p.setJointMotorControl2(panda, right_finger_id, p.POSITION_CONTROL, targetPosition=0,
                            force=new_force, maxVelocity=target_velocity)
            print(new_force)
            for j in range(240):
                p.stepSimulation()
            ...
            log_1=record_decrease_force(panda,object,log_1,center_of_mass)
            log_1.to_csv(f'{output_path}_{pose_num}_decrease_froce.csv', index=True,na_rep='NA')
...
# 获得特定位置接触点的信息
def get_grasp_link_contact_points(panda_id, object_id,i):

    contact_points = p.getContactPoints(panda_id, object_id,i)  # 获取机械臂与积木的接触点
    contact_info = []
...
#初始化json
def initialization_json():
    # 初始化 JSON 文件
    with open(f"{trans_json_path}.json", "w") as json_file:
        json.dump([], json_file)  # 写入空数组
    return json_file
...
def record(panda_id, object,log,center_of_mass,flag):
    ...
    if len(contact_points) > 0:
        ...
        for contact in contact_points:
                # log.at[log.index[-1], '接触点位置'].append(contact['position_object'])
                ...
                # #计算接触点相对于质心的距离
                # dictance_contact,gravity_ori_contact=calculate_distance_and_gravity_ori_world(contact['position_object'],object_pos)
                # log.at[log.index[-1], '相对于物体质心的距离'].append(dictance_contact)
                # #计算接触点在物体坐标系的位置
                contact_position=globalpoint_T_object(contact['position_object'],object_T_global)
...
#记录抓取力降低过程中的信息
def record_decrease_force(panda_id, object,log_1,center_of_mass):
    # 获取接触点信息
    contact_points = get_grasp_contact_points(panda_id, object)
...
def read_json(i):
    ...
    return grasp_center,grasp_endpoints,gravity,flag,obj_id,scene_id,cam_id
...
if __name__=="__main__":
    ...
    parser.add_argument("-m", "--modle",help="被抓物体obj文件路径", default="obj/scene_0100/58_simple.obj")
    parser.add_argument("-g", "--grasp_json",help="grasp.json文件路径", default="data.json")
    parser.add_argument("-op", "--output_path",help="输出结果文件路径", default="output_simple/scene_0100/0058/0058")
    parser.add_argument("-transjson", "--trans_json_path",help="输出json文件路径", default="data.json")
    ...
    # 定义文件夹路径
    json_dir = "json"  # JSON 文件所在目录
    obj_dir = "obj"    # OBJ 文件所在目录
    output_dir = "new_json"  # 输出结果保存目录

    # 遍历 JSON 文件夹的子文件夹
    for scene_folder in sorted(os.listdir(json_dir)):
        if scene_folder == "scene_0111":
            json_folder_path = os.path.join(json_dir, scene_folder)
            obj_folder_path = os.path.join(obj_dir, scene_folder)
            output_folder_path = os.path.join(output_dir, scene_folder)
        else:
            continue
        # 如果文件夹不存在，跳过
        if not os.path.isdir(json_folder_path) or not os.path.isdir(obj_folder_path):
            continue
        ...
        # 遍历 JSON 文件夹中的文件
        for json_file in sorted(os.listdir(json_folder_path)):
            if json_file.endswith(".json"):
                # JSON 文件路径
                json_file_path = os.path.join(json_folder_path, json_file)
                # 提取 JSON 文件名中的关键字（不含扩展名）
                json_key = json_file.split('_')[-1].split('.')[0] 
                json_key_1 = json_file.split('.')[0]
                # 找到对应的 OBJ 文件
                matched_obj_file = None
                for obj_file in os.listdir(obj_folder_path):
                ...
                if not matched_obj_file:
                    print(f"Warning: No matching OBJ file for {json_file}")
                    continue
                ...
                # 读取 OBJ 文件内容
                obj_file_path = os.path.join(obj_folder_path, matched_obj_file)
                with open(obj_file_path, 'r', encoding='utf-8') as of:
                    obj_data = of.readlines()

                # 将处理后的数据写入到对应的输出文件夹
                output_scene_subfolder = os.path.join(output_folder_path, json_key_1)  # 按数字保存
                #os.makedirs(output_scene_subfolder, exist_ok=True)
                output_file_path = output_scene_subfolder

                mesh_path=obj_file_path
                json_path=json_file_path
                output_path=output_file_path
                trans_json_path=output_file_path
                center_of_mass,total_area=get_center_of_mass_from_mesh()
                print(f"处理{json_file}和{matched_obj_file}，输出结果到{output_file_path}")
