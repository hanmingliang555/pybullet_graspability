...
def run():
    ...
    for i in range(1):
        ...
        if flag_1==True:
                ...
                #逐渐减小机械臂的力，记录最小的夹取力
                mass=increase_mass(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,obj_key,center_of_mass)
                print(mass)
                #移除所有物体、机械臂、辅助线
                if object==-1:
                    p.removeBody(panda)
...
def increase_mass(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,pose_num,center_of_mass):
    if flags==True:
        ...
        # 每1秒质量增加0.1千克
            ...
            middle_joint_states = p.getJointState(panda, right_finger_id)
...
if __name__=="__main__":
    ...
    out_put_path_base = "alldata/output/increase_mass/change_obj"
    # 遍历 rts 文件夹的子文件夹
    for sub_folder in os.listdir(rts_folder):
        if sub_folder=="FR-20-0.005":
