...
def run():
    ...
    for i in range(1):
        ...
        grasp_center,grasp_endpoints,gripper_ori,gravity,flag_1,obj_id,scene_id=read_json()
        ...
        if flag_1==True:
                ...
                #逐渐减小机械臂的力，记录最小的夹取力
                min_force=decrease_force(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,obj_key,center_of_mass)
                print(min_force)
                #移除所有物体、机械臂、辅助线
                if object==-1:
                    p.removeBody(panda)
...
def generate_urdf(template_path, output_path,grasp_endpoints,gripper_ori,j):
    ...
    output_path=os.path.join(output_path,method_key,grasp_key,part)
...
#在稳定后逐渐减小机械臂的力
def decrease_force(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,pose_num,center_of_mass):
    if flags==True:
        new_force=50
        ...
        # 每1秒减少N的力
            ...
            middle_actual_force = middle_joint_states[3]  # 第四个值是实际施加的力/力矩
            print(f"Actual force: {middle_actual_force}")
            object_pos_2,object_ori=p.getBasePositionAndOrientation(object)
            obj_center_move_distance = np.linalg.norm(np.array(object_pos_1)-np.array(object_pos_2)) 
            matrix_2 = quaternion_to_rotation_matrix_panda(object_ori)
            #计算稳定前后末端执行器移动的空间角（弧度值）
            ...
            log_1.to_csv(f'{output_path}_{pose_num}_decrease_force.csv', index=True,na_rep='NA')
...
if __name__=="__main__":
    ...
    # 设置 rts 文件夹路径
    rts_folder = "rst_0116_0"  # 替换为实际路径
    out_put_path_base = "alldata/output/decrease_force/change_obj"
    # 遍历 rts 文件夹的子文件夹
    for sub_folder in os.listdir(rts_folder):
        if sub_folder=="FC-20-0.005":
            sub_folder_path = os.path.join(rts_folder, sub_folder)
            method_key=sub_folder.split("-")[0]
            out_put_path_method=os.path.join(out_put_path_base,method_key)
            os.makedirs(out_put_path_method,exist_ok=True)
            # 检查是否是文件夹
            if os.path.isdir(sub_folder_path):
                # 遍历子文件夹中的子子文件夹
                for sub_sub_folder in os.listdir(sub_folder_path):
                    if "obj" in sub_sub_folder:
                        start_idx = sub_sub_folder.index("obj") + 3
                        end_idx = sub_sub_folder.index("-", start_idx)
                        obj_number = sub_sub_folder[start_idx:end_idx]
                        if obj_number =="0" :
                            # 转换为四位格式
                            formatted_number = f"{int(obj_number):04d}"
                            grasp_key=sub_sub_folder.split("-")[-1]
                            out_put_path_num=os.path.join(out_put_path_method,obj_number,grasp_key)
                            os.makedirs(out_put_path_num,exist_ok=True)
                            sub_sub_folder_path = os.path.join(sub_folder_path, sub_sub_folder)
                            
                            if os.path.isdir(sub_sub_folder_path):
                                # 找到 grasp.json 文件
                                json_path = os.path.join(sub_sub_folder_path, "grasp.json")
                                if os.path.exists(json_path):
                                    ...
                                    for file_name in os.listdir(sub_sub_folder_path):
                                        if file_name.endswith(".obj"):
                                            mesh_path = os.path.join(sub_sub_folder_path, file_name)
                                            part = "_".join(file_name.split("_")[2:4]).split("obj")[0]
                                            output_path=os.path.join(out_put_path_num,part)

                                            obj_key+=1
                                            center_of_mass,total_area=get_center_of_mass_from_mesh()
                                            print(f"Processed: {json_path},{mesh_path} -> {output_path}")
                                            run()
