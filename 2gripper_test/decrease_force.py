    ...
    for i in range(100):
        ...
        if flag_1==True:
            for j in range(num_points):
                ...
                #逐渐减小机械臂的力，记录最小的夹取力
                min_force=decrease_force(flags,panda,object,object_pos_1,matrix_1,log_1,obj_id,scene_id,i,center_of_mass)
                print(min_force)
                #移除所有物体、机械臂、辅助线
                if object==-1:
                    p.removeBody(panda)
...
def close_finger(panda):
    ...
    p.setJointMotorControl2(panda, left_finger_id, p.POSITION_CONTROL, targetPosition=0,force=target_force,targetVelocity=0.2)

    p.setJointMotorControl2(panda, right_finger_id, p.POSITION_CONTROL, targetPosition=0,force=target_force,targetVelocity=0.2)
    for i in range(720):
        p.stepSimulation()
...
if __name__=="__main__":
    ...
    start_time=time.time()
    base_dir = "2gripper_data"
    json_base = "json"
    obj_base = "obj"
    output_base = "output/decrease_force"
    ...
    for scene_folder in sorted(os.listdir(json_dir)):
            if scene_folder.endswith(".json") and scene_folder=="0000_0021.json":
                # JSON 文件路径
